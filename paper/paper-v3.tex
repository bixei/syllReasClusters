...
Originally, one of the objectives of using (not necessarily classical) logic in the field of AI and logic programming was to formalize human reasoning and commonsense reasoning~\cite{McCarthy59,McCarthy98}. 
However, the proposed approaches did not have the expected successes in this~(cf.~\cite{mcdermott:87}),
as proposed approaches were mostly theoretical and did not apply to real case studies.
...
Yet, the cognitive adequacy of a theory can only be measured, if that theory is evaluable. In this context, that means, if the theory can uniformally account for episodes of human reasoning systematically, according to its underlying assumptions. Only then, the cognitve adequacy of that theory can be compared to others.

Formal methods can offer the required tools to implement such cognitive theories. This in turn, can allow a new way of communication among the fields, by avoiding ambiguous understanding, showing their formal correspondences and finally providing an appropriate evaluation method with respect to their cognitive adequacy. In order to determine the required factors that are necessary for the evaluation of an adequate cognitive theory, we take~\cite{ragni:stolzenburg:2015} as a starting point:
...
were not the same ones who answered \emph{No chemists are artists} or \emph{Some artists are not chemists}, and vice versa.\footnote{Note that usually in these experiments, participants were supposed to give only one answer}
...
The authors in~\cite{declare:2017} put forward the hypothesis that reasoning clusters can be expressed through cognitive principles and heuristic strategies. Cognitive principles are (not necessarily valid) assumptions made by humans while reasoning, 
...
$A$ is an atom and the~$L_i$ with~$1 \leq i \leq m+p$ is a literal.
...
It also corresponds to the Aristotelian interpretation~\cite{parry:1991} and is assumed by several cognitive theories (cf.~\cite{johnsonlaird:1983,Rips1994}). 
...
By requiring for \MI\ and \MO, the existence of an object with the first property,
for which the second property has to be either either false or unknown, \MA\ and \MI, and, \ME\ and \MO, exclude each other, respectively.
...
\end{description}
Given this sobering outcome of the non-aggregated evaluation, 
...
Thanks to the reviewer for pointing us to the relationship of the $\cxt$ operator
with {\L}ukasiewicz' extensional necessity operator.
