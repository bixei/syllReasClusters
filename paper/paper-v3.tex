...
it seems that already in 1913, Kotarbi{\'n}ski~\cite{kotarbinski:1913} \textit{suggested
...
Since then, various cognitive theories have been proposed,
 but it seems that none of them is generally accepted
within the community. As various experimental studies have shown that human reasoning systematically deviates from conclusions that are valid under CL (cf.~\cite{wason:68,byrne:89}),
cognitive scientists 
seem at least to agree with Kotarbi{\'n}ski,
 that classical (two-valued)
logic would not be an adequate candidate for a cognitive theory.
...
that performed well on aggregated data, and investigate whether these clusters are also adequate with respect to the non-aggregated dataset given by Johnson-Laird and Khemlani~\cite{khemlani:2016}.
...
\footnotetext{Thanks to the
 anonymous reviewer for the observation, that the $\cxt$ operator seems to be a special case of {\L}ukasiewicz necessity operator~\cite[p. 20]{malinowski:1993}, restricted to literals.
 }
...
\subsection{On aggregated Data}\label{sect:aggregateddata}
...
Taking the non-aggregated data from~\cite{khemlani:2016} as benchmark,
we investigated how well their responses could be predicted through the cognitive principles,
and whether the individual participants' responses would consistently correspond to a certain set of principles. Recall, their dataset consisted of responses from 20 participants, with each two entries,
...
We identified three clusters, using the same labels as in~\cite{khemlani:2016}: 
...
Two of the three clusters can be specified through a set of cognitive principles, but for one
...
is characterized by the participants whose responses corresponded to the predictions given by abduction/ generalization. Almost none of their conclusions corresponded to the predictions made by contraposition (except for 2 participants in week 2). They showed a high change within their own responses from week 1 to week 2 
 ...
 \item[Intermediate cluster] (7 out of 20 participants) is characterized by the participants whose responses did not corresponded to any particular set of principles. These participants had a high change within their own responses from week 1 to week~2
