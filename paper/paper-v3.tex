...
Originally, one of the objectives of using logic in the field of AI and logic programming was to formalize human reasoning and commonsense reasoning~\cite{McCarthy59,McCarthy98}. However, it did not have the expected successes~(cf.~\cite{mcdermott:87}).
Although several alternatives have been proposed, such as non-monotonic or many-valued logics, the emphasis had shifted away from their application to real case studies of human reasoning.
On the other hand, cognitive scientists have extensively investigated human thinking and acquired a great expertise.
However, this expertise was not consulted, i.e. there was no communication among the relevant fields. 

Following~\cite{strube:1992}, we argue that a theory that aims at modeling episodes of human reasoning needs to be evaluable according to its cognitive adequacy and should be comparable and compared to other cognitive theories. 
...
Formal methods can offer the required tools to implement elaborate cognitive theories. This in turn, can allow a new way of communication among the fields, by avoiding ambiguous understanding of the theories, showing their formal correspondences and finally providing an appropriate evaluation method with respect to their cognitive adequacy. In order to determine the required factors that are necessary for the evaluation of an adequate cognitive theory, we take~\cite{ragni:stolzenburg:2015} as a starting point:
We aim for a theory in which (i) all processes are comprehensible, (ii) the suggested principles represent general cognitive processes, and (iii) the predictions of our theory are evaluated with respect to a wide range of human reasoning tasks. 
...
% from classical logical conclusions (cf.~\cite{wason:68,byrne:89}).

The results of the meta-study on syllogistic reasoning in~\cite{khemlani:2012} show that humans do not only systematically deviate from the assertions in CL, but from any other proposed cognitive theory (such as the theories proposed in~\cite{BJ89,johnsonlaird:1983,Rips1994,Polk1995,chater:oaksford:1999}).
...
\begin{array}{@{\hspace{0.0cm}}l@{\hspace{0.1cm}}G@{\hspace{0.1cm}}l@{\hspace{0.3cm}}c}\toprule
	\mbox{mood} & \mbox{natural language} & \mbox{FOL} &
\mbox{Short} \\\midrule
...
%  	\small
	...
	\[
		...
		\toprule
		& \mbox{premise 1} & \mbox{premise 2} \\
		\midrule
		\mbox{figure 1} &  a-b & b-c \\
		\mbox{figure 2} & b-a &  c-b \\
		\mbox{figure 3} & a-b & c-b \\
		\mbox{figure 4} & b-a & b-c \\
		\bottomrule
		\end{array}
		\]
	\end{center}
...
concluded \emph{No chemists are artists} (29\%) and \emph{No valid conclusion} (27\%). The classical logical valid conclusion \emph{Some artists are not chemists}, was concluded 
by only 16\%, which is exactly the threshold, that the answer had not been chosen randomly.\footnote{
...
were not the same ones who answered \emph{No chemists are artists} or \emph{Some artists are not chemists}, and vice versa.
...
As the above example and recent investigations on psychological experiments show (cf.~\cite{khemlani:2016, ragni:2017}), it seems that different humans represent and reason about conditional information differently.
...
In Section~\ref{sect:wcsclustering} and~\ref{sect:representing}, we introduce the underlying computational logic approach and the cognitive principles. Section~\ref{sect:aggregateddata} illustrates an example and briefly discusses the performance of the approach in~\cite{declare:2017} on non-aggregated data.
Section~\ref{sect:nonag} provides a new characterization of the individual's reasoning patterns that 
seems to be more adequate.

\section{WCS Clustering} \label{sect:wcsclustering}

\cite{declare:2017} puts forward the hypothesis that reasoning clusters can be expressed through cognitive principles and heuristic strategies. Cognitive principles are (not necessarily valid) assumptions made by humans while reasoning, 
...
The approach suggested in~\cite{declare:2017} is based on a new computational logic approach for human syllogistic reasoning~\cite{cogsci:2017}, which seems to perform better than other state-of-the-art cognitive theories when evaluated with aggregated data from~\cite{khemlani:2012}. \cite{declare:2017} extended the approach in~\cite{cogsci:2017} by specifying three reasoning clusters and two heuristic strategies and achieved a match of 92\%, which was the highest result compared to the presented approaches in~\cite{khemlani:2012}.
The system developed in~\cite{declare:2017}, was extended and participated at the Syllogism Challenge 2017.\footnote{\url{https://www.cc.uni-freiburg.de/modelingchallenge/challenge-2017}} Figure~\ref{overview:system:syllchal} illustrates the components of the system:
First, groups of reasoners were specified based on cognitive principles and heuristic strategies.
After that, 
a translation into logic program clauses was provided, such that for each reasoning clusters, the corresponding logic program
was constructed. Next, the desired models according to the Weak Completion Semantics~\cite{hk:2009a,hk:2009b} were computed with a corresponding semantic operator~\cite{stenning:vanlambalgen:2008}.
After that, the entailed relation between $a$ and $c$ were extracted. If no relation between $a$ and $c$ could be entailed, then \textit{No valid conclusion}
was derived. The final answer distribution was specified according to the number of reasoning clusters 
that entailed the conclusion.
Finally, an illustrative representation of these clusters with respect to their distribution was done by means of of multinomial process trees~\cite{Riefer1988}.
...
As environment for the formalization, we suggest an extension of the
Weak Completion Semantics (WCS). WCS has first been introduced in~\cite{hk:2009a,hk:2009b} 
and has shown to model a wide range of human reasoning tasks~\cite{btg:2015:h}. 
...
\textit{Contextual logic programs} are (data) logic programs extended by the truth-functional operator~$\cxt$, called \textit{context}. \textit{Contextual (logic) program clauses}
...
assumption will become $A \leftrightarrow \bot$ and, hence, $A$ has to be false.
This can, however, be overwritten by other rules and facts (\textit{defeating} the assumption).} and $A \leftarrow \udf$ (called
\textit{unknown assumptions}), where
$A$ is an atom and the~$L_i$ with~$1 \leq i \leq m+p$ are
literals.
...
$A$ is \textit{defined in $\Ground \CalP$}  iff $\Ground \CalP$ contains a rule, a fact 
or an assumption with head $A$.
The \textit{definition of $A$ in $\Ground \CalP$} is defined as $\Def(A,\CalP)
= \{ A \leftarrow \Body \mid A \leftarrow \Body \mbox{ is a rule, a fact or an assumption
...
\end{array}
\]
\caption{The truth tables for the connectives under the three-valued {\L}ukasiewicz logic and for $\cxt(L)$. $L$ is a literal, $\top$, $\bot$, and $\udf$ denote \textit{true}, \textit{false},
...
\subsection{Reasoning with Respect to the Least Fixed Point of $\Svlp$}


For a given $\CalP$, consider the following transformation:
1.\ For each ground atom~$A$ which occurs as head of a clause in $\Ground \CalP$, replace
...
of~$\CalP$ or~$\WC\CalP$.
Consider the following semantic operator, which is due to Stenning and van Lambalgen~\cite{stenning:vanlambalgen:2008}: Let $I = \langle I^\top,I^\bot
...
\end{array}\Shrink
\]
\cite{hk:2009a} showed that the
weak completion of non-contextual programs\footnote{Non-contextual programs refer programs that do not contain rules, in which $\cxt$ appears in the body of any rule, i.e.\ $p = 0$.} 
always has a least model under  {\L}ukasiewicz logic,
which can be computed by the least fixed point of~$\Svlp$. The correspondence to other well-known logic programming approaches, in particular 
...
if these programs do not contain cycles, then there always exists a supported model, which can be 
computed by iterating $\Svlp$: A least fixed point of $\Svlp$ always exists for contextual programs
that do not contain cycles~\cite{dietz:hoelldobler:pereira:2017}.
...
Note that for all programs that are considered in the following, a least fixed point of $\Svlp$ exists.
...
\subsubsection{Conditional representation, license for inference and existential import}
...
However, it seems that humans do not understand it as \textit{strict}, but defeasible~\cite{stenning:vanlambalgen:2008},
i.e.\ they implicitly assume a license for the inference (\licenses). For (\MA yz) this can be 
...
It is assumed by several cognitive theories (cf.~\cite{johnsonlaird:1983,Rips1994}). 
 Likewise,~\cite{stenning:vanlambalgen:2008} have shown that humans
require existential import for a conditional to be true.
$\CalP_{Ayz}$, the logic program representing (\MA yz) is as follows:
...
\end{align}
Note that $z'(X)$ appears within the $\cxt$ operator in the body of the second clause, where $z'$ represents the negation of $z$:\footnote{$z'(X)$ can be introduced by the negation by transformation principle in the following.}
...
% \end{align}
% \]
We obtain \[\CalM_{\CalP_{Ayz}} = \langle \{y(o),z(o)\}, \{\Ab_{yz}(o)\} \rangle.\]


\subsubsection{Negation by transformation and no double negation}
...
Another principle that we introduce is the no double negation principle: Under the Weak Completion Semantics, a positive conclusion can be derived from double negation using two conditionals. However, it appears to be the case that humans do not reason in such a way~\cite{khemlani:2012}. Hence, we block them with the help of abnormalities (\dnegation). $\CalP_{Eyz}$, the logic program representing (\ME yz) is as follows:
...
\subsubsection{Converse implication and unknown generalization}
...
$y$ and for which it is unknown whether it belongs to~$z$.
$\CalP_{Iyz}$, the logic program representing (\MI yz), is as follows:\footnote{For simplicity, here we omit the clauses introduced by ({\small\converse}). They can be found in~\cite{declare:2017}.}
\begin{align}
...
As in the previous cases, (\conditionals), (\licenses) and (\unknownGen) apply.
Furthermore, as the conclusion is again a negation of a claim, we need to assume (\transformation) and
(\dnegation). $\CalP_{Oyz}$, the logic program representing (\MO yz) is as follows:
\begin{align}
z'(X) & \leftarrow y(X) \wedge \neg \Ab_{ynz}(X). & \tag{\conditionals\ \&\ \transformation \ \&\ \licenses} \\
...
but characterize the reasoning clusters, that will be specified in the following section.
...
There is
evidence that some of the participants make use of this
equivalence when solving syllogistic reasoning tasks~\cite{khemlani:2012}. We believe that when they
encounter a premise with the mood \MA\ (e.g., \textit{all~a are~b}),
then might reason with the
contrapositive conditional as well (\contraposition).
...
\subsubsection{Generalization}  %\label{sect:naf}

If all of the principles introduced so far are applied to an existential
premise, the only object about which an inference can be made is the one
resulting from (\import). This is because the
abnormality introduced by (\licenses) and according to (\unknownGen)
...
circumstances~\cite{khemlani:2012}, i.e.\ they generalize (universally quantify) the existential 
...
$\Ab_{yz}(o_2) \leftarrow \udf$ guarantees that $\Ab_{yz}(o_2)$ stays unknown, even though $\cxt(z'(o_2))$ might false.
...
\subsubsection{Search for Alternative Conclusions}
...
We use only the first as source for observations, as they are used directly to infer new information. 
Consider the following pair of syllogistic premises: 
\begin{align}
...
these bakers are artists. Under WCS, we formalize this reasoning procedure by means of skeptical abduction for logic programming~\cite{kakas:etal:93}.
Due to lack of space we will not introduce the formal abductive approach for human syllogistic reasoning in this paper.
The interested reader is referred to~\cite{cogsci:2017}.
...
the following condition needs to hold: There exists an object $o_2$ such that $\CalP \ModelsWCS
...
z(o_3) \wedge y(o_3)$ and there exists an
object $o_4$ such that $\CalP \ModelsWCS z(o_4)$ and
$\CalP \not\ModelsWCS y(o_4)$.
...
object $o_2$ such that $\CalP \ModelsWCS y(o_2)$ and
$\CalP \not\ModelsWCS \neg z(o_2)$.
\item[\NVC] When no previous conclusion can be derived, no valid conclusion holds.
\end{description}



\section{Preliminary Evaluation} \label{sect:aggregateddata}

Let us again consider \mbox{\MI\ME 2} from the introduction: 
\begin{align}
...
and \emph{Some artists are not chemists} (16\%): Participants who concluded the first answer, did not conclude the other two answers, and vice versa.
In order to account for these individual differences, three clusters on principles and two on heuristic strategies have been suggested
...
The interested reader is referred to~\cite{declare:2017} for details on their application on the syllogistic reasoning task. 
}
The characterization of the three clusters specified through the cognitive principles is shown Table~\ref{tab:cluster}, together with the conclusions for \MI\ME 2 according to each cluster. 
\begin{table}
\begin{center}
\begin{tabular}{@{\hspace{0cm}}l@{\hspace{0.9cm}}c@{\hspace{0.9cm}}c@{\hspace{0.9cm}}cGG}
Principle & Basic & Contraposition & Generalization & Heuristic 1 & Heuristic 2 \medskip \\ \midrule  
...
{\small\textsf{contraposition}} & - & $\checkmark$            & -            & - & - \smallskip\\\midrule
...
Applying the contraposition principle to \textit{No chemists are bakers} yields to \textit{No bakers are chemists}.
...
yields to \textit{All bakers are artists} and \textit{All artists are bakers}.
...
for all $o$ for which $c(o)$ is true,
which only applies for $c(o_5)$, we find that $a(o_5)$ is false.

WCS clustering~\cite{declare:2017} suggests three reasoning clusters characterized by the following principles:\footnote{\textsf{basic} refers to the set of basic principles introduced in Subsection~\ref{sect:basic}} (1) \basic, \abduction, and \converse\ for \MI,
    (2) \basic, \converse\ for \MI\ and \negFailure, and
    (3) \basic, \converse\ for \MI, \ME, and \contraposition\ for~\MA.\footnote{\cite{declare:2017} also included two heuristic clusters based on
         atmosphere bias~\cite{Woodworth1935} and matching bias~\cite{Wetherick1995}.}
...
We were interested whether these clusters were also cognitively adequate in the sense that they 
would also correspond to the reasoning patterns of individuals. Therefore, we investigated the non-aggregated dataset provided by~\cite{khemlani:2016} consisting of the responses of 20 participants for two weeks,
i.e. the participants carried out the task twice, with a break of one week. The results with respect to WCS clustering is two-fold:
\begin{description}
 ...
 all majority's conclusions were predicted by WCS clustering.
\item[Non-aggregated evaluation] None of the clusters fitted any individual participants' response pattern, i.e.\ no participant did belong to a single cluster.
\end{description}
Given this sobering outcome of (2), we investigated whether patterns could be found on the level of the
...
\section{On non-aggregated Data} \label{sect:nonag}
...
\end{tabular}
\caption{Overview of each participants' performance with respect to the specified criteria in week 1 and week 2.
...
for which the task was carried twice, with a break of one week. On average, the basic principles explained the results on about 10\% better than CL.
...
A natural explanation for this difference, could be that~\cite{khemlani:2016}
...
is characterized by the participants whose answers corresponded to the predictions given by abduction/ generalization. Almost none of their conclusions corresponded to the predictions made by contraposition (except of 2 participants in week 2). They showed a high change within their own answers from week 1 to week 2 
 ...
 \item[Intermediate cluster] (7/20) is characterized by the participants whose answers did not corresponded to any particular principle. These participants had a high change within their own answers from week 1 to week 2
 and no particular pattern with respect to NVC conclusions. Generally they were not improving their answers from week 1 to week 2 with respect to CL, but normally compensated for 
 right answers in some cases (that corresponded to contraposition) with wrong answers in the other cases (that correspond to abduction/ generalization).
\end{description}

 \section{Conclusions and Future Directions}
 
 
We have investigated the cognitive adequacy of WCS clustering with respect to the individual answer patterns reported by~\cite{khemlani:2016}.
...
This in turn might give us a new starting point to investigate whether the cognitive principles  specified here can characterize participants' conclusions in other reasoning tasks.
% episodes.
