...
\todo[inline]{R: Für mich ist der letzte Satz unverständlich. Vorschlag: ``\dots each of these properties occurs in exactly one of the premises, and there is a third property connecting both premises which does not occur in the conclusion.''}\noindent%
\begin{table}[t]
...
Next, the desired\todo{R: desired? Entailed, oder was willst du sagen?} models were computed (Section~\ref{sect:wcs}-\ref{sect:lfp}).
...
\todo[inline]{R: Ist es eine Überlegung wert, \L ukasiewicz logic vor contextual programs einzuführen? Oben benutzen wir schon $\udf$, was erst hier eingeführt wird.}
...
literals\todo{R: ``and $\udf$ denotes unknown'' entfernt, sollte an dieser Stelle klar sein}. An interpretation maps
...
if these programs do not contain cycles, then there always exists a supported model\todo{R: Begriff ``supported model'' unklar}, which 
...
specified using the predicates $y$ and $z$, where~$yz$ can be replaced by $ab$, $ba$, $cb$, or $bc$ and depend on the figures shown in Table~\ref{t:2}. First, we introduce the basic principles that are assumed for all premises when applicable. Thus, we assume them to be assumed\todo{R: assumed $\to$ applied?} by all (human) reasoners. 
...
However, it seems that humans understand them defeasibly, \todo{R: i.e.?} they assume a \textit{license for the inference}~\cite{stenning:vanlambalgen:2008},
...
apply the contrapositive (\contraposition) within a reasoning task (cf.~\cite{rips:1994,mentallogic:1994})\todo{R: Die Referenzen gibt es bei mir nicht}.
...
By requiring for \MI yz and \MO yz the existence of an object $o_1$ with the first property $y$,
for which the second property $z$ has to be either either false or unknown,
\todo[inline]{R: Das ist doch nicht richtig, oder? Bei \MI yz muss $z(o_2)$ entweder false oder unknown sein, wie beschrieben, aber bei \MO yz muss es entweder true oder unknown sein.}
...
\todo[inline]{R: Ich finde, hier wird nicht ganz klar, was das ``level of cognitive principles'' ist}
...
They\todo{R: statt ``they'' \cite{khemlani:2016}} specified four parameters for the construction of a mental model and evaluated their
...
size of a mental model, (ii) the probability of drawing\todo{R: was heißt draw hier?} the complete set, 
(iii) the probability to search for counter examples,
and (iv) the probability to weaken the conclusion.
The clustering with mReasoner was based on partitioning around\todo{R: around \emph{the} medoids?} medoids clustering algorithm to determine the optimal number of clusters.
...
Another interesting aspect is to investigate in how far the semantics of the $\cxt$ operator within the extended three-valued {\L}ukasiewicz logic in Section~\ref{sub:3valuedL} 
 corresponds to the {\L}ukasiewicz extensional necessity operator~\cite{}\todo{R: Referenz fehlt}.\footnote{Thanks to the
